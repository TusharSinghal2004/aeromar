        ...
        <div className="grid lg:grid-cols-[1.6fr_0.9fr] gap-10">
          ...
          <div ref={leftRef}>
            ...
            <div style={fadeUp(leftInView, 0.05)} className="flex items-center gap-3 mb-3">
              ...
              <span className="uppercase tracking-[0.2em] text-[#1A73C8] text-[10px] font-bold">
                Overview
              </span>
            </div>
            ...
            <h2
              ...
              className="text-3xl md:text-4xl font-extrabold text-[#1E3A7B] leading-tight max-w-3xl"
            >
            ...
            {/* Description */}
            <p
              style={fadeUp(leftInView, 0.25)}
              className="mt-5 text-gray-600 text-base leading-8 max-w-4xl"
            >
            ...
            {/* Capabilities */}
            <div className="mt-10">
              <div style={fadeUp(leftInView, 0.32)}>
                <h3 className="uppercase tracking-[0.2em] text-[#1A73C8] text-[11px] font-bold mb-5">
                  Capabilities
                </h3>
              </div>
              ...
              <div className="space-y-4">
                {capabilities.map((item, index) => (
                  <div
                    key={index}
                    style={fadeUp(leftInView, 0.38 + index * 0.08)}
                    className="flex items-start gap-3"
                  >
                    <CheckCircle size={18} className="text-[#C8960A] mt-1 shrink-0" />
                    <p className="text-[#081229] text-base">{item}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div ref={rightRef}>
            <div
              style={fadeUp(rightInView, 0.1)}
              className="bg-[#F8F9FB] p-6 md:p-8"
            >
              ...
              <div className="space-y-5 mb-8">
                {benefits.map((item, index) => (
                  <div
                    key={index}
                    style={fadeUp(rightInView, 0.2 + index * 0.08)}
                    className="flex items-start gap-3"
                  >
                    <span className="w-2 h-2 rounded-full bg-[#C8960A] mt-3 shrink-0" />
                    <p className="text-[#081229] text-base leading-7">{item}</p>
                  </div>
                ))}
              </div>

              <div style={fadeUp(rightInView, 0.55)}>
                <Link
                  to="/quote"
                  className="
                    w-full h-12 bg-[#1E3A7B] text-white font-semibold text-sm
                    flex items-center justify-center gap-3
                    hover:bg-[#163164] transition-all
                  "
                >
                  REQUEST A QUOTE
                  <ArrowRight size={16} />
